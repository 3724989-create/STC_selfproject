...
static const uint8 Guess_kernel[Guess_kernel_size]={5,5,5,5,5};
...
void ccd_process_data(void)
{
    ...
    // 压缩系数为 4096 / 128 = 32
    uint8 j;
    const uint16 COMPRESSION_DIVER=32;
    const uint8 MAX_NOMPRESSED_VALUE=128;
...
{
    uint8 i,j;
    const u16 TSL1401_MAX_VALUE=225;
    const u8 COMPRESSION_DIVER=32;
  for(j = 0; j < 128; j++)
  {
    y1_boundary[j] =(u8)(TSL1401_MAX_VALUE-tsl1401_data[0][j] / COMPRESSION_DIVER);
  }
   
  //apply_frame_averaging(tsl1401_data[0]); //计算量太大丢数据
  //apply_gussian_smoothing(averaged_pixels,y1_boundary,TSl140_LEN);
  //高斯模糊算法有问题，不适合ccd识别，直接识别效果就很好了，可能是计算量太大尝试的数据缺少问题
