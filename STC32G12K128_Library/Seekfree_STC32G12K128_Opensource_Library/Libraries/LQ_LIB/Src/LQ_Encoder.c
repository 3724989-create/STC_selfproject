...
//    Motor_Init();         //如果检测，电机转动情况下编码器的读取情况，可以配合去掉下面有关电机的注释
    while (1)
    {
...
//        Motor_Ctrl(duty,duty);
        ...
        sprintf(txt, "Enc1: %04d  ", Read_Encoder(Enc_1));//左电机 母板上编码器1，小车前进为负值
        OLED_P6x8Str(0, 3, txt);			        //字符串显示
//        sprintf(txt, "PWM: %05d;", duty);
//        OLED_P6x8Str(0, 4, txt);   //字符串显示
        sprintf(txt, "Enc2: %04d  ", Read_Encoder(Enc_2));//右电机 母板上编码器2，小车前进为正值
