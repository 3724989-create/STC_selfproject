...
------------------------------------------------------------------------------------
 * 使用龙邱母板测试电机及驱动流程：
 * 1.先使用万用表测量电池电压，务必保证电池电压在7V以上，否则可能无力不反应！
 * 2.接好母板到驱动板的信号线及电源线；
 * 3.接好驱动板到电机的导线；
 * 4.烧写程序并运行，确定电机能正常转动后，开启驱动板电源开关；
 * 5.按键K1/K3确定电机转动速度及方向；
...
void Test_Motor (void)
{
  ...
  //如果是STC32,选择PWM管脚后 需要对应修改GIO初始化，以下主要针对AI8051U，使用STC32G的自行修改
...
//  printf("F:%.0f\n",MAIN_Fosc/12500.0);
  while (1)
  {
    ...
    {
		if (duty > -3360)               // 满占空比为3360 @42 MHz主频 MAIN_Fosc/12500
        duty -= 50;
    }
...
void Test_Servo(void)
{
  ...
  while (1)
  {
    ...
    sprintf(txt, "S_duty:%05d   ", duty);
    OLED_P8x16Str(1, 3, txt); 	// 显示
    sprintf(txt, "S_Med:%05d ", Servo_Median);//显示当前宏定义舵机中值
    OLED_P8x16Str(1, 5, txt); 	// 显示
...
//  Servo_Ctrl(Servo_Center); 	// 舵机中值
  ...
  OLED_Init();
  OLED_CLS();   				// 清屏
  OLED_P8x16Str(0, 0, "LQ BLSmotor Test");
