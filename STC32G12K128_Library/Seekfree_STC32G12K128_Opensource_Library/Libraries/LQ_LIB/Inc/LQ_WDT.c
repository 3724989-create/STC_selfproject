 ...
 *      分频系数参数    分频系数   42MHz下时间       其他频率下的时间自根据公式计算
 *      WDT_SCALE_2,       0         约0.018 s  
 *      WDT_SCALE_4,       1         约0.037 s
 *      WDT_SCALE_8,       2         约0.075 s
 *      WDT_SCALE_16,      3         约0.150 s
 *      WDT_SCALE_32,      4         约0.300 s
 *      WDT_SCALE_64,      5         约0.599 s
 *      WDT_SCALE_128,     6         约1.198 s
 *      WDT_SCALE_256      7         约2.397 s
 ***************************************************************************************/
...
void Test_Sample_WDT(void)
{
    u8 i= 0;
    GPIO_LED_Init();
    WDT_Init(WDT_SCALE_32);    //分频系数参数，时间约300ms

// 初始化完后LED1 闪烁，表示程序复位初始化了

    for(i=0;i<10;i++)       //复位后执行LED快闪
    {
        LED_Ctrl(LED0,RVS);
        delay_ms(80);
        Feeding_Dog();
    ...
    while(1)
    {
        ...
        delay_ms(315);      //延时超过300ms,则会触发看门狗 复位，
		LED_Ctrl(LED0,RVS);
    }
}
