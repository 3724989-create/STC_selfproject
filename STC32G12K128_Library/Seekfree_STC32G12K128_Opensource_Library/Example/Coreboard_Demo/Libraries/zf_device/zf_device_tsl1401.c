...
void tsl1401_binary_data_process(uint8 index,uint8 i,uint16 threshold);
...
void tsl1401_collect_pit_handler (void)
{
    ...
    for(i = 0; i < TSL1401_DATA_LEN; i ++)
    {
        ...
        tsl1401_binary_data_process(0,i,255);
        TSL1401_CLK(1);
    }
    //采集完成标志位置1
    tsl1401_finish_flag = 1;    //对图像元素进行处理
}
...
void tsl1401_binary_data_process(uint8 index,uint16 threshold)  //100 130
{
     for(i = 0; i < TSL1401_DATA_LEN; i ++)
    {
        if(tsl1401_data[index][i]>threshold)
        {
            tsl1401_data[index][i]=130;  // 白色或反光
        }
        else
        {
              tsl1401_data[index][i]=100;  // 黑色或无反射
        }
    }
