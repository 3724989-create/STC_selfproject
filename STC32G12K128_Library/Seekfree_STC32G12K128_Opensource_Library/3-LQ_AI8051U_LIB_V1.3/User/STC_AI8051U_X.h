...
//包含STC32G、32F、AI8051U_32Bit寄存器,STC32G系列,STC32F,AI8051U_32Bit系列均可使用此头文件
/*  BYTE Registers  */
sfr         P0          =           0x80;
sbit        P00         =           P0^0;
sbit        P01         =           P0^1;
...
sfr         RSTCFG      =           0xff;
sbit        ENLVR       =           RSTCFG^6;
sbit        PNRST       =           RSTCFG^4;   //统一复位寄存器名称 STC32G/F -->P54、AI8051U-->P47
...
#define     HPLLCR                  (*(unsigned char volatile far *)0x7efe0c)   //STC32F 支持
#define     HPLLPSCR                (*(unsigned char volatile far *)0x7efe0d)   //STC32F 支持
...
//2024-11-15    添加 8051U 定时器T11，寄存器地址,已修改TX ,后续沿用
#define     TxR                     0x80      //T11CR^7;计数控制寄存器 0：停止计数，1：开始计数
#define     Tx_CT                   0x40      //T11CR^6;工作模式寄存器 0：做定时器，1：做计数器P1.4
#define     TxCLKO                  0x20      //T11CR^5;时钟输出寄存器 0：关闭时钟输出， 1：使能时钟输出 P1.5
#define     TxX12                   0x10      //T11CR^4;分频控制寄存器 0：12T模式12分频，1：不分频1T模式
#define     TxCSH                   0x08      //T11CR^3;T11CR^2;  [1:0]  T11时钟源选择   默认00 系统时钟
#define     TxCSL                   0x04      //T11CR^2;11CR^1;定时/计数时钟选择 00：系统时钟，01：内部高速IRC,10：外部32K，11：内部低速IRC
#define     ETxI                    0x02      //T11CR^1;T11CR^0; 定时/计数中断允许位：0：禁止中断，1：允许中断
#define     TxIF                    0x01      //T11CR^0;溢出中断标志位 当产生溢出时由硬件置1，由硬件清0（也可软件查询或清0）
...
//2024-11-15    添加 8051U SPI,I2S相关寄存器地址
...
// #define     DMA_I2SR_TXAH           (*(unsigned char volatile far *)0x7efabd)   //STC32F 寄存器名不同
// #define     DMA_I2SR_TXAL           (*(unsigned char volatile far *)0x7efabe)   //STC32F
