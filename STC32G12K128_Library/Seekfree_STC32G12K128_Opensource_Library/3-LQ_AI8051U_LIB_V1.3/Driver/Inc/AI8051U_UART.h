...
#endif
#ifdef UART2
#define	COM_TX2_Lenth	64         //设置串口2发送数据缓冲区大小，如果长度定义大于256，需要将程序中与该长度比较的变量类型由u8改为u16.
#define	COM_RX2_Lenth	64         //设置串口2接收数据缓冲区大小，如果长度定义大于256，需要将程序中与该长度比较的变量类型由u8改为u16.
#endif
#ifdef UART3
...
#ifdef UART1
void UART1_PutBuf(u8 dat);	//串口1发送函数
void UART1_PutStr(u8 *puts);
#endif
#ifdef UART2
void UART2_PutBuf(u8 dat);	//串口2发送函数
void UART2_PutStr(u8 *puts);
#endif
#ifdef UART3
void UART3_PutBuf(u8 dat);	//串口3发送函数
void UART3_PutStr(u8 *puts);
#endif
#ifdef UART4
void UART4_PutBuf(u8 dat);	//串口4发送函数
void UART4_PutStr(u8 *puts);
#endif
