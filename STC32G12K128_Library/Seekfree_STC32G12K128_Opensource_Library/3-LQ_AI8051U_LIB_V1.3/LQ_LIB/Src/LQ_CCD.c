...
void CCD1_Init() 
{
    ...
    P1_MODE_IN_HIZ(GPIO_Pin_7);     //ADC管脚，高阻态输入
    ...
    P1_MODE_OUT_PP(GPIO_Pin_5);     //推挽输出
    P1_MODE_OUT_PP(GPIO_Pin_3);     //推挽输出
}
...
void Test_CCD()
{
	...
	while(1)
	{
        /* 采集ccd图像 */
        CCD_Collect(ccd_buf);
        ccd_binaryzation(THRESHOLD, ccd_buf, CCD1_Bin); // 　二值化并滤波,耗时约0.1ms
        /* OLED 显示 */
        ...
        for(i=0; i<128; i++)
        {
            ...
            if(CCD1_Bin[i] > 0)	   //显示二值化后的	
                OLED_WrDat(0xFF);
            else
                OLED_WrDat(0x00);	            
//            if(ccd_buf[i] > THRESHOLD)//显示原始     			
//                OLED_WrDat(0xFF);
//            else
//                OLED_WrDat(0x00);	
        }
