...
//龙邱OLED软件SPI口模块管脚顺序  更换管脚需要同步修改OLED_Init()中初始化的IO
...
void OLED_Init(void)
{
    GPIO_InitTypeDef	OLED_SPI_Structure;
    //初始化OLED软件SPI使用收发管脚，需要与实际选择的串口保持一致
    OLED_SPI_Structure.Pin  = GPIO_Pin_1 | GPIO_Pin_5 | GPIO_Pin_6; //指定要初始化的IO
    OLED_SPI_Structure.Mode = GPIO_PullUp;                          //指定IO内部上拉，准双向口模式
    GPIO_Inilize(GPIO_P2,&OLED_SPI_Structure);                      //初始化，结构体配置

    OLED_SPI_Structure.Pin  = GPIO_Pin_2 | GPIO_Pin_4;              //指定要初始化的IO
    OLED_SPI_Structure.Mode = GPIO_PullUp;                          //指定IO内部上拉，准双向口模式
    GPIO_Inilize(GPIO_P3,&OLED_SPI_Structure);
...
//	delay_ms(1000);
	...
	delay_ms(500);
	OLED_CLS();
//	OLED_P14x16Str(8,0,(unsigned char*)"北京龙邱智能科技");//字符串显示,起始位置坐标，字符内容

	while (1)
	{
//        OLED_P14x16Str(8,0,(unsigned char*)"北京龙邱智能科技");
